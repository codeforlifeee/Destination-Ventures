    ...
    const loadData = async () => {
      try {
        const [packagesData, bannersData] = await Promise.all([
          fetchPackagesByCategory('kerala'),
          fetchBanners('kerala')
        ]);
        setPackages(packagesData);
        setBanners(bannersData);
      } catch (error) {
        console.error('Failed to load Kerala packages:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen pt-20 pb-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-darkBlue font-canva-sans">Loading Kerala packages...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 z-10 w-11/12 max-w-3xl">
            <div className="bg-white rounded-2xl p-4 md:p-5 shadow-2xl border border-lightGray">
              <div className="text-center mb-4">
                <h1 className="text-2xl md:text-3xl lg:text-4xl font-season font-bold text-darkBlue">Kerala Holidays Packages</h1>
                <p className="text-darkBlue/70 mt-2 font-canva-sans">Experience God's Own Country with backwaters and hill stations</p>
              </div>
              <form
              ...
              >
                <input
                  ...
                  placeholder="Search Kerala packages..."
                  value={query}
        ...
        <div className="container mx-auto px-4">
          {filtered.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((pkg) => (
                <PackageCard key={pkg._id} pkg={pkg} category="kerala" />
              ))}
            </div>
          ) : (
