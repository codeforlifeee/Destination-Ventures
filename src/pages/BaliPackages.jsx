    ...
    const loadData = async () => {
      try {
        const [packagesData, bannersData] = await Promise.all([
          fetchPackagesByCategory('bali'),
          fetchBanners('bali')
        ]);
        setPackages(packagesData);
        setBanners(bannersData);
      } catch (error) {
        console.error('Failed to load Bali packages:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen pt-20 pb-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-darkBlue font-canva-sans">Loading Bali packages...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="container mx-auto px-4">
          {filtered.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((pkg) => (
                <PackageCard key={pkg._id} pkg={pkg} category="bali" />
              ))}
            </div>
          ) : (
